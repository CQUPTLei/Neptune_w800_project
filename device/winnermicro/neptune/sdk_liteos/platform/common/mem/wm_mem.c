...
#if TLS_OS_FREERTOS// USING_ADD_HEADER
...
#elif TLS_OS_LITEOS
    mem_re_addr = LOS_MemRealloc(m_aucSysMem0, mem_address, size);
#else
	tls_os_sem_acquire(mem_sem, 0);
	cpu_sr = tls_os_set_critical();
...
#elif TLS_OS_LITEOS
    buffer = LOS_MemAlloc(m_aucSysMem0, n*size);
#else   //UCOSII
	tls_os_sem_acquire(mem_sem, 0);
    cpu_sr = tls_os_set_critical();
