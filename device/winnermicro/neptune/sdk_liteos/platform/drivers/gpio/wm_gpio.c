...
void tls_gpio_set_dir(enum tls_io_name gpio_pin, enum tls_gpio_dir dir)
{
	u8	pin;
	u16 offset;
...
void tls_gpio_get_dir(enum tls_io_name gpio_pin, enum tls_gpio_dir *dir)
{
	u8	pin;
	u16 offset;
	...
	if (tls_reg_read32(HR_GPIO_DIR + offset) & BIT(pin))
		*dir = 0;
	else
		*dir = 1;
}

void tls_gpio_get_pull(enum tls_io_name gpio_pin, enum tls_gpio_attr *attr)
{
	u8	pin;
	u16 offset;
	u32 up, down;
	...
	up = tls_reg_read32(HR_GPIO_PULLUP_EN + offset) >> pin; 
	down = tls_reg_read32(HR_GPIO_PULLDOWN_EN + offset) >> pin;
...
void tls_gpio_set_fun(enum tls_io_name gpio_pin)
{
	u8	pin;
	u16 offset;
...
void tls_gpio_get_fun(enum tls_io_name gpio_pin)
{
	u8	pin;
	u16 offset;
