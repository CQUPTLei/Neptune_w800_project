...
/* check interrupt status */
    ...
    if ((intr_src & UART_RX_INT_FLAG) && (0 == (port->regs->UR_INTM & UIS_RX_FIFO)))
    {
        ...
        while (rx_fifocnt-- > 0)
        {
            ...
            if (CIRC_SPACE(recv->head, recv->tail, port->recv.buffer_size) <= 2)
            {
        ...
        /* insert the character into the buffer */
            ...
            recv->head = (recv->head + 1) & (port->recv.buffer_size - 1);
            rxlen++;
        }
        ...
        if (port->rx_block && rxlen) {
            tls_os_sem_release(port->rx_sem);
        }
    }
...
ATTRIBUTE_ISR void UART1_IRQHandler(void)
{
    ...
    if ((intr_src & UART_RX_INT_FLAG) && (0 == (port->regs->UR_INTM & UIS_RX_FIFO)))
    {
        ...
        if (CIRC_SPACE(recv->head, recv->tail, port->recv.buffer_size) <= RX_CACHE_LIMIT)
        {
            recv->tail = (recv->tail + RX_CACHE_LIMIT) & (port->recv.buffer_size - 1);
        }
        ...
        if( escapefifocnt==3 && ch=='+')
        {
            ...
            {
                case 0:
                    if(recv->buf[port->recv.buffer_size-1]=='+' && recv->buf[port->recv.buffer_size-2]=='+')
                        port->plus_char_cnt = 3;
                    break;
                case 1:
                    if(recv->buf[0]=='+' && recv->buf[port->recv.buffer_size-1]=='+')
                        port->plus_char_cnt = 3;
                    break;               
                default:
...
void tls_uart_gpio_config(u16 uart_no)
{
    switch (uart_no)
    {
        case 0:
...
int tls_uart_port_init(u16 uart_no, tls_uart_options_t * opts, u8 modeChoose)
{
    ...
    if (NULL == opts)
    {
        ...
        opt.rx_buffer_size = TLS_UART_RX_BUF_SIZE;
        opt.tx_buffer_size = TLS_UART_TX_BUF_SIZE;
        opts = &opt;
    ...
    else
    {
        if (opts->rx_buffer_size == 0) {
            opts->rx_buffer_size = TLS_UART_RX_BUF_SIZE;
            opts->tx_block = 0;
            opts->rx_block = 0;
            opts->tx_use_dma = 0;
            opts->rx_use_dma = 0;
            opts->tx_buffer_size = TLS_UART_RX_BUF_SIZE;
        }
    ...
    {
    	bufrx = tls_mem_alloc(opts->rx_buffer_size);
    	if (!bufrx)
    	    return WM_FAILED;
        memset(bufrx, 0, opts->rx_buffer_size);
    	port->recv.buf = (u8 *) bufrx;	
        port->recv.buffer_size = opts->rx_buffer_size;
    }
    port->recv.head = 0;
    port->recv.tail = 0;
    port->tx_fifofull = 16;
    port->tx_block = opts->tx_block;
    port->rx_block = opts->rx_block;
    port->tx_use_dma = opts->tx_use_dma;
    port->rx_use_dma = opts->rx_use_dma;
    port->tx_buffer_size = opts->tx_buffer_size;
    ...
    if (port->rx_block) {
        if (tls_os_sem_create(&port->rx_sem, 1)) {
            return WM_FAILED;
        }
    }

    if (port->tx_block) {
        if (tls_os_sem_create(&port->tx_sem, 1)) {
            return WM_FAILED;
        }
    }
...
int tls_uart_try_read(u16 uart_no)
{
	...
	port = &uart_port[uart_no];
    recv = &port->recv;
    data_cnt = CIRC_CNT(recv->head, recv->tail, port->recv.buffer_size);
...
int tls_uart_read(u16 uart_no, u8 * buf, u16 readsize)
{
    ...
    recv = &port->recv;
    data_cnt = CIRC_CNT(recv->head, recv->tail, port->recv.buffer_size);
    if (data_cnt == 0 && port->rx_block) {
        tls_os_sem_acquire(port->rx_sem, 0);
        data_cnt = CIRC_CNT(recv->head, recv->tail, port->recv.buffer_size);
    }
    (data_cnt >= readsize)?(buflen = readsize):(buflen = data_cnt);
    if ((recv->tail + buflen) > port->recv.buffer_size)
    {
        bufcopylen = (port->recv.buffer_size - recv->tail);
    ...
    recv->tail = (recv->tail + buflen) & (port->recv.buffer_size - 1);
    return buflen;
}
...
int tls_uart_dma_write(char *buf, u16 writesize, void (*cmpl_callback) (void *p), u16 uart_no)
{
    ...
    /* Request DMA Channel */
    dmaCh = tls_dma_request(2 /* 2 Gary fix */, TLS_DMA_FLAGS_CHANNEL_SEL(TLS_DMA_SEL_UART_TX) | TLS_DMA_FLAGS_HARD_MODE); 
    if (dmaCh != 2)
    {
        TLS_DBGPRT_ERR("\ndma request err\n");
        return WM_FAILED;
    }
    port->tx_dma_ch = dmaCh;
    tls_reg_write32((int)&port->regs->UR_DMAC, (tls_reg_read32((int)&port->regs->UR_DMAC) & ~0x01));
...
static void tls_uart_dma_tx_callback(void *p)
{
    struct tls_uart_port *port = &uart_port[(u32)p];
    if (port->tx_block) {
        tls_os_sem_release(port->tx_sem);
    }
    tls_dma_free(port->tx_dma_ch);
...
int tls_uart_write_async(u16 uart_no, char *buf, u16 writesize)
{
    ...
    port = &uart_port[uart_no];
    if (port->tx_use_dma) {
        ret = tls_uart_dma_write(buf, writesize, tls_uart_dma_tx_callback, uart_no);
    } else {
