...
void ble_server_register_app_cb(int status, int server_if, uint16_t app_uuid)
{
    ...
    tls_ble_server_add_service(server_if, 1, 1, app_uuid16_to_uuid128(gatt_uuid[WM_WIFI_SERVICE_INDEX].uuid), gatt_uuid[WM_WIFI_SERVICE_INDEX].numHandles);
}
...
void ble_server_service_added_cb(int status, int server_if, int inst_id, bool is_primary, uint16_t app_uuid, int srvc_handle)
{
    if(status != 0)
    {
        TLS_HAL_CBACK(ps_wifi_prof_callback, enabled_cb, status);
    ...
    tls_ble_server_add_characteristic(server_if, srvc_handle, app_uuid16_to_uuid128(gatt_uuid[WM_WIFI_CHARC_INDEX].uuid),  gatt_uuid[WM_WIFI_CHARC_INDEX].properties, gatt_uuid[WM_WIFI_CHARC_INDEX].permissions);
}
...
void ble_server_characteristic_added_cb(int status, int server_if, uint16_t char_id, int srvc_handle, int char_handle)
{
    if(status != 0)
    {
        TLS_HAL_CBACK(ps_wifi_prof_callback, enabled_cb, status);
    ...
    tls_ble_server_add_descriptor(server_if, srvc_handle, app_uuid16_to_uuid128(gatt_uuid[WM_WIFI_DESC_INDEX].uuid), gatt_uuid[WM_WIFI_DESC_INDEX].permissions);
}
