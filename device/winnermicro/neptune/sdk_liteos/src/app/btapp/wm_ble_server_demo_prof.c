...
void ble_demo_server_service_added_cb(int status, int server_if, int inst_id, bool is_primary, uint16_t app_uuid, int srvc_handle)
{
	TLS_BT_APPL_TRACE_DEBUG("%s...status=%d, server_if=%d,uuid=0x%04x srvc_handle=%d\r\n",__FUNCTION__, status, server_if, app_uuid, srvc_handle);
...
void ble_demo_server_characteristic_added_cb(int status, int server_if, uint16_t char_id, int srvc_handle, int char_handle)
{
