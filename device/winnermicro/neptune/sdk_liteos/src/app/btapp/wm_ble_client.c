...
void btgattc_read_descriptor_callback(int conn_id, int status,uint16_t handle, uint8_t* value, int len, int value_type, int rstatus)
{
    int index = -1;
...
static void tls_ble_client_event_handler(tls_ble_evt_t evt, tls_ble_msg_t *msg)
{
	...
	switch(evt)
	{
		case WM_BLE_CL_REGISTER_EVT:
			btgattc_register_client_callback(msg->cli_register.status, msg->cli_register.client_if, &msg->cli_register.app_uuid);
			break;
		...
		case WM_BLE_CL_OPEN_EVT:
			...
			btgattc_connect_callback(msg->cli_open.conn_id, msg->cli_open.status, msg->cli_open.client_if, &addr);
			break;
		case WM_BLE_CL_CLOSE_EVT:
			memcpy(addr.address, msg->cli_close.remote_bda, 6);
			btgattc_disconnect_callback(msg->cli_close.conn_id, msg->cli_close.status, msg->cli_close.reason, msg->cli_close.client_if, &addr);
			break;
		...
		case WM_BLE_CL_SEARCH_RES_EVT:
			btgattc_search_service_result_callback(msg->cli_search_res.conn_id, &msg->cli_search_res.uuid, msg->cli_search_res.inst_id);
			break;
		case WM_BLE_CL_REG_NOTIFY_EVT:
			btgattc_register_for_notification_callback(msg->cli_reg_notify.conn_id, msg->cli_reg_notify.reg, msg->cli_reg_notify.status, msg->cli_reg_notify.handle);
			break;
		case WM_BLE_CL_NOTIF_EVT:
			memcpy(addr.address, msg->cli_notif.bda, 6);
			btgattc_notify_callback(msg->cli_notif.conn_id, msg->cli_notif.value, &addr, msg->cli_notif.handle, msg->cli_notif.len, msg->cli_notif.is_notify);
			break;
		case WM_BLE_CL_READ_CHAR_EVT:
			btgattc_read_characteristic_callback(msg->cli_read.conn_id, msg->cli_read.status, msg->cli_read.handle, msg->cli_read.value, msg->cli_read.len, msg->cli_read.value_type, msg->cli_read.status);
			break;
		case WM_BLE_CL_WRITE_CHAR_EVT:
			btgattc_write_characteristic_callback(msg->cli_write.conn_id, msg->cli_write.status, msg->cli_write.handle);
            break;
		...
		case WM_BLE_CL_READ_DESCR_EVT:
			btgattc_read_descriptor_callback(msg->cli_read.conn_id, msg->cli_read.status, msg->cli_read.handle, msg->cli_read.value, msg->cli_read.len, msg->cli_read.value_type, msg->cli_read.status);
			break;
		case WM_BLE_CL_WRITE_DESCR_EVT:
			btgattc_write_descriptor_callback(msg->cli_write.conn_id, msg->cli_write.status, msg->cli_write.handle);
			break;
		...
		case WM_BLE_CL_CFG_MTU_EVT:
			btgattc_configure_mtu_callback(msg->cli_cfg_mtu.conn_id, msg->cli_cfg_mtu.status, msg->cli_cfg_mtu.mtu);
			break;
		...
		case WM_BLE_CL_REPORT_DB_EVT:
			btgattc_get_gatt_db_callback(msg->cli_db.status, msg->cli_db.conn_id, msg->cli_db.db, msg->cli_db.count);
			break;
		default:
