...
void btgatts_service_added_cb(int status, int server_if, uint8_t inst_id, uint8_t is_primary, tls_bt_uuid_t * uuid,  int srvc_handle)
{
    ...
    TLS_HAL_CBACK(app_env[index].ps_callbak, service_added_cb, status, server_if, inst_id, is_primary, app_uuid, srvc_handle);
}
                                       ...
                                       int incl_srvc_handle)
{
    ...
    TLS_HAL_CBACK(app_env[index].ps_callbak, included_service_added_cb, status, server_if, srvc_handle, incl_srvc_handle);
}

void btgatts_characteristic_added_cb(int status, int server_if, tls_bt_uuid_t *char_id, int srvc_handle, int char_handle)
{
    int index = -1;
    uint16_t app_uuid;
    index = get_app_env_index_by_server_if(server_if);
                             ...
                             int attr_handle, int offset, bool is_long)
{
    ...
    TLS_HAL_CBACK(app_env[index].ps_callbak, request_read_cb, conn_id, trans_id, bda, attr_handle, offset, is_long);
}
                              ...
                              bool need_rsp, bool is_prep, uint8_t *value)
{
    ...
    if(need_rsp)
    {
		tls_ble_server_send_response(conn_id,trans_id,TLS_BT_STATUS_SUCCESS,offset,attr_handle,0,value,length);
    }
    ...
    TLS_HAL_CBACK(app_env[index].ps_callbak, request_write_cb, conn_id, trans_id, bda, attr_handle, offset, length, need_rsp, is_prep, value);
}
                                   ...
                                   tls_bt_addr_t *bda, int exec_write)
{
    ...
    if(index<0)
    {
    	TLS_BT_APPL_TRACE_ERROR("%s ,conn_id = %d, trans_id = %d, exec_write = %d\r\n", __FUNCTION__, conn_id, trans_id, exec_write);
    	return;
    }
...
static void tls_ble_server_event_handler(tls_ble_evt_t evt, tls_ble_msg_t *msg)
{
	...
	switch(evt)
	{
		case WM_BLE_SE_REGISTER_EVT:
			btgatts_register_app_cb(msg->ser_register.status, msg->ser_register.server_if, &msg->ser_register.app_uuid);
			break;
		...
		case WM_BLE_SE_ADD_CHAR_EVT:
			btgatts_characteristic_added_cb(msg->ser_add_char.status, msg->ser_add_char.server_if, &msg->ser_add_char.uuid, msg->ser_add_char.service_id, msg->ser_add_char.attr_id);
			break;
		case WM_BLE_SE_ADD_CHAR_DESCR_EVT:
			btgatts_descriptor_added_cb(msg->ser_add_char_descr.status, msg->ser_add_char_descr.server_if, &msg->ser_add_char_descr.uuid, msg->ser_add_char_descr.service_id, msg->ser_add_char_descr.attr_id);
			break;
		case WM_BLE_SE_START_EVT:
			btgatts_service_started_cb(msg->ser_start_srvc.status, msg->ser_start_srvc.server_if, msg->ser_start_srvc.service_id);
			break;
		case WM_BLE_SE_STOP_EVT:
			btgatts_service_stopped_cb(msg->ser_stop_srvc.status, msg->ser_stop_srvc.server_if, msg->ser_stop_srvc.service_id);
			break;
		case WM_BLE_SE_DELETE_EVT:
			btgatts_service_deleted_cb(msg->ser_delete_srvc.status, msg->ser_delete_srvc.server_if, msg->ser_delete_srvc.service_id);
			break;
		case WM_BLE_SE_READ_EVT:
			memcpy(addr.address, msg->ser_read.remote_bda, 6);
			btgatts_request_read_cb(msg->ser_read.conn_id, msg->ser_read.trans_id, &addr,msg->ser_read.handle, msg->ser_read.offset,msg->ser_read.is_long);
			break;
		case WM_BLE_SE_WRITE_EVT:
			memcpy(addr.address, msg->ser_write.remote_bda, 6);
			btgatts_request_write_cb(msg->ser_write.conn_id, msg->ser_write.trans_id, &addr,msg->ser_write.handle, msg->ser_write.offset,msg->ser_write.len, msg->ser_write.need_rsp, msg->ser_write.is_prep, msg->ser_write.value);
			break;
		case WM_BLE_SE_EXEC_WRITE_EVT:
			memcpy(addr.address, msg->ser_exec_write.remote_bda, 6);
			btgatts_request_exec_write_cb(msg->ser_exec_write.conn_id, msg->ser_exec_write.trans_id, &addr, msg->ser_exec_write.exec_write);
			break;
		...
		case WM_BLE_SE_RESP_EVT:
			btgatts_response_confirmation_cb(msg->ser_resp.status, msg->ser_resp.conn_id, msg->ser_resp.trans_id);
			break;
		...
		case WM_BLE_SE_ADD_INCL_SRVC_EVT:
			btgatts_included_service_added_cb(msg->ser_add_incl_srvc.status, msg->ser_add_incl_srvc.server_if, msg->ser_add_incl_srvc.service_id, msg->ser_add_incl_srvc.attr_id);
			break;
		default:
